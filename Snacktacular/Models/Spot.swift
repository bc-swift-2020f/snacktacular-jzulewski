...
class Spot: NSObject, MKAnnotation {
    ...
    func updateAverageRating(completed: @escaping() -> ()) {
        ...
        reviewsRef.getDocuments { (querySnapshot, error) in
            guard error == nil else {
                print("Error: failing to get query snapshot of reviews for reviewsRef \(reviewsRef)")
                return completed()
            }
            ...
            let dataToSave = self.dictionary // creates dictionary
            ...
            spotRef.setData(dataToSave) { (error) in
                if let error = error {
                    ...
                    print("New averagee \(self.averageRating). Document updated with ref ID \(self.documentID)")
                    completed()
                }
            }
        }
