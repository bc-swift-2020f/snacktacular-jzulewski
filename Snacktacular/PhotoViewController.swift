...
class PhotoViewController: UIViewController {
    ...
    func updateUserInterface() {
        ...
        if photo.documentID == "" { //New Review
            addBordersToEditableObjects()
        } else {
            if photo.photoUserID == Auth.auth().currentUser?.uid { //current photo posted
                self.navigationItem.leftItemsSupplementBackButton = false
                saveBarButton.title = "Update"
                addBordersToEditableObjects()
                self.navigationController?.setToolbarHidden(false, animated: true)
            } else { // different photo
                saveBarButton.hide()
        ...
        guard let url = URL(string: photo.photoURL) else {
            // Then this must be a new image, so get the image from the photo.image passed in rather in te url
            photoImageView.image = photo.image
            return
        }
        photoImageView.sd_imageTransition = .fade
        photoImageView.sd_imageTransiton?.duration = 0.5
    ...
    @IBAction func saveButtonPressed(_ sender: UIBarButtonItem) {
        ...
        photo.saveData(spot: spot) { (success) in
            if success {
                ...
                print("Error: Can't unwind segue from PhotoReviewController because of photo saving error")
            }
        }
        leaveViewController()
    }
